...
namespace em {
    ...
    template<typename APHValueType_, size_t rank_> class RealObject;
    
    template<typename APHValueType_, size_t rank_>
    class ComplexObject {
    public:

        typedef multidim::Tensor<Complex<APHValueType_>, rank_, multidim::StorageOrder::COLUMN_MAJOR> container_type;
        typedef Complex<APHValueType_> value_type;
        ...
        range_type logical_range() const {
            ...
            if (even_size_x_) range[0] = (range[0] - 1)*2;
            else range[0] = range[0]*2 - 1;
            return range;
        }
