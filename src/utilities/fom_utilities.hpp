    ...
    namespace utilities {
        namespace fom_utilities {
            ...
            double FomToXarg(double fom) {
                ...
                double fom_in = LOOKUP_FOM_TO_XARG[fom_position][0];

                //get the correct FOM position
                while (fom_in > fom) {
                    fom_in = LOOKUP_FOM_TO_XARG[fom_position][0];
                    fom_position--;
                }

                //Linear interpolation
                xarg = (LOOKUP_FOM_TO_XARG[fom_position][1])
                        + (LOOKUP_FOM_TO_XARG[fom_position + 1][1] - LOOKUP_FOM_TO_XARG[fom_position][1])
                        * ((fom - LOOKUP_FOM_TO_XARG[fom_position][0]) / (LOOKUP_FOM_TO_XARG[fom_position + 1][0] - LOOKUP_FOM_TO_XARG[fom_position][0]));
            ...
            double XargToFom(double xarg) {
                ...
                double i0 = em::utilities::bessel_functions::i0(xarg);

                //Calculate the Bessel function I1(x)
                double i1 = em::utilities::bessel_functions::i1(xarg);
             ...
             * the average of XARG. 
             * @param foms: vector of FOM values in fractions
             * @return averaged weight
             */
            double AverageFOMs(const std::vector<double>& foms) {
                //Get the sum of XARGS from weights
                double xarg_sum = 0.0;
                for (const auto& itr : foms) {
                    double xarg = em::utilities::fom_utilities::FomToXarg(itr);
                    xarg_sum += xarg;
                }
                ...
                double avg_weight = em::utilities::fom_utilities::XargToFom(xarg_sum);
